<div>
        ...
        <div class="fixed inset-0 bg-black/60 admin-modal-overlay flex items-center justify-center z-50 animate-fade-in-scale" style="animation: fadeInScale 0.3s ease-out;">
            <div class="bg-white rounded-xl shadow-2xl max-w-sm mx-4 transform transition-all scale-100 animate-scale-in">
                <div class="p-6">
                    ...
                    <p class="text-gray-600 mb-6 pl-16">{{ $isEditing ? 'Obat berhasil diperbarui.' : 'Obat berhasil ditambahkan.' }}</p>
        ...
        <div class="fixed inset-0 bg-black/70 admin-modal-overlay flex items-center justify-center z-[9999] p-4 animate-fade-in-scale" style="animation: fadeInScale 0.3s ease-out;">
            <div class="bg-white rounded-xl shadow-2xl max-w-md mx-4 transform transition-all scale-100 animate-scale-in">
                <div class="p-6">
                    ...
                    <p class="text-gray-600 mb-6 pl-16">Apakah Anda yakin ingin menghapus obat ini? Tindakan ini tidak dapat dibatalkan.</p>
            ...
            <div class="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col transform scale-100 animate-scale-in">
                ...
                <div class="bg-gradient-to-r from-[#004643] to-[#003d3a] px-6 py-4 flex items-center justify-between relative overflow-hidden">
                    ...
                    <div class="relative z-10 flex items-center justify-between w-full">
                        <div>
                            <h2 class="text-2xl font-bold text-white">{{ $isEditing ? 'Edit Obat' : 'Tambah Obat Baru' }}</h2>
                            <p class="text-white/80 text-sm mt-1">{{ $isEditing ? 'Perbarui informasi obat' : 'Masukkan informasi obat baru' }}</p>
                        </div>
                ...
                <div class="p-6 overflow-y-auto flex-1">
                    <form id="medicine-form" wire:submit.prevent="save" class="space-y-6">
                        <div class="grid md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Nama Obat <span class="text-red-500">*</span></label>
                                <input type="text" wire:model="name" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-white" placeholder="Masukkan nama obat" required>
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="number" wire:model="stock" min="0" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-white" placeholder="Masukkan jumlah stok" required>
                                @error('stock') <span class="text-red-500 text-sm flex items-center mt-1"><svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path></svg>{{ $message }}</span> @enderror
                            </div>
                            <div class="space-y-2 md:col-span-2">
                                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Deskripsi <span class="text-red-500">*</span></label>
                                <textarea wire:model="description" rows="3" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 bg-white resize-none" placeholder="Deskripsi obat" required></textarea>
                                @error('description') <span class="text-red-500 text-sm flex items-center mt-1"><svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path></svg>{{ $message }}</span> @enderror
                            </div>
                            <div class="space-y-2 md:col-span-2">
                                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Gambar Obat</label>
                                <div class="relative">
                                    <input type="file" wire:model="image" wire:key="medicine-image-{{ $editingId ?? 'new' }}" accept="image/*" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 file:mr-4 file:py-1.5 file:px-4 file:rounded file:border-0 file:text-sm file:font-medium file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 bg-white">
                                    @if($image)
                                        <p class="text-sm text-green-600 mt-1">✓ File dipilih: {{ $image->getClientOriginalName() }}</p>
                                    @endif
                                </div>
                ...
                <div class="bg-gray-50 px-6 py-4 border-t border-gray-200 flex justify-end space-x-3">
                    ...
                    <button type="submit" form="medicine-form" class="admin-btn-primary inline-flex items-center px-5 py-2.5 text-white font-medium rounded-lg shadow-sm">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                        </svg>
                        {{ $isEditing ? 'Update Obat' : 'Simpan Obat' }}
                    </button>
                </div>
            </div>
        </div>
    ...
    <div class="mb-6 fade-in-section">
        <button type="button" wire:click="create" class="admin-btn-primary inline-flex items-center px-5 py-2.5 text-white font-medium rounded-lg shadow-sm transform hover:scale-105 transition-all duration-300 relative overflow-hidden group">
            ...
            <span class="relative z-10">Tambah Obat Baru</span>
        </button>
    </div>
    ...
    <div class="admin-card p-6 {{ $showModal || $showSuccessModal || $showDeleteModal ? 'pointer-events-none opacity-50' : '' }} transition-all duration-300">
        <div class="flex items-center justify-between mb-6 pb-4 border-b border-gray-200">
            <div>
                ...
                <p class="text-gray-600 text-sm mt-1">Kelola semua obat yang terdaftar</p>
            </div>
            <div class="flex items-center space-x-4">
                <div class="text-sm text-[#004643] bg-[#004643]/10 border border-[#004643]/20 px-4 py-2 rounded-lg">
                    <span class="font-semibold text-[#004643]">{{ $medicines->count() }}</span>
                    <span class="text-[#004643]/70"> obat</span>
                </div>
            </div>
        </div>
        ...
        <div class="overflow-x-auto -mx-6 px-6">
            <table class="admin-table min-w-full">
                <thead>
                    <tr>
                        <th class="text-left py-3 px-4 font-semibold text-gray-700 text-sm uppercase tracking-wider">Obat</th>
                        <th class="text-left py-3 px-4 font-semibold text-gray-700 text-sm uppercase tracking-wider">Harga</th>
                        <th class="text-left py-3 px-4 font-semibold text-gray-700 text-sm uppercase tracking-wider">Stok</th>
                        ...
                        <tr class="border-b border-gray-100">
                            <td class="py-4 px-4">
                                <div class="flex items-center">
                                    @if($medicine->image)
                                        <img src="{{ asset('storage/' . $medicine->image) }}" alt="{{ $medicine->name }}" class="w-12 h-12 rounded-lg object-cover mr-3 border-2 border-gray-200">
                                    @else
                                    ...
                                    <div>
                                        ...
                                        @if($medicine->description)
                                            <div class="text-sm text-gray-500 truncate max-w-xs mt-0.5">{{ Str::limit($medicine->description, 50) }}</div>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="py-4 px-4">
                                <span class="inline-flex items-center px-3 py-1 rounded-md text-sm font-medium {{ $medicine->stock > 0 ? 'bg-green-100 text-green-800 border border-green-200' : 'bg-red-100 text-red-800 border border-red-200' }}">
                                    {{ $medicine->stock }} unit
                                </span>
                            </td>
                            <td class="py-4 px-4">
                                <div class="flex space-x-2">
                                    <button type="button" wire:click="edit({{ $medicine->id }})" class="inline-flex items-center px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-white text-sm font-medium rounded-lg transition-all duration-300 shadow-sm hover:shadow-lg transform hover:scale-110 hover:-translate-y-1">
                                    ...
                                    <button type="button" wire:click="confirmDelete({{ $medicine->id }})" class="inline-flex items-center px-3 py-1.5 bg-red-500 hover:bg-red-600 text-white text-sm font-medium rounded-lg transition-all duration-300 shadow-sm hover:shadow-lg transform hover:scale-110 hover:-translate-y-1">
                                        <svg class="w-4 h-4 mr-1 transform group-hover:rotate-12 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                        </svg>
                                        Hapus
                        ...
                        <tr>
                            <td colspan="4" class="py-16 px-4 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-gray-400 text-sm mt-1">Klik tombol "Tambah Obat Baru" untuk menambahkan obat pertama</p>
                                </div>
                            </td>
                        </tr>
                    @endif
